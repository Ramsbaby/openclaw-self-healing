#!/bin/bash
# Gateway Watchdog v5.6 - Emergency Recovery 실제 구현
#
# v5.6 개선사항 (2026-02-09):
# - **Emergency Recovery 실제 호출**: doctor --fix 실패 시 emergency-recovery.sh 백그라운드 실행
# - **Claude Code PTY 세션**: 자동으로 진단 및 복구 요청
# - **100% 자가복구 완성**: 모든 설정 손상 상황에서 자동 복구 가능
#
...
if [[ "$pid_status" == "NOT_LOADED" ]] || [[ "$pid_status" == STOPPED:* ]] || [[ "$pid_status" == CRASHED:* ]]; then
    ...
    if [[ $crash_count -ge 2 ]]; then
        ...
        if [[ $attempts -lt $DOCTOR_FIX_MAX_ATTEMPTS ]]; then
            ...
            # v5.6: doctor --fix 실패 → Level 3 Emergency Recovery 호출
            log "WARN" "doctor --fix 실패 - Level 3 Emergency Recovery 시작"

            # Option 1: LaunchAgent 사용 (더 안정적)
            EMERGENCY_PLIST="$HOME/Library/LaunchAgents/ai.openclaw.emergency-recovery.plist"
            if [[ -f "$EMERGENCY_PLIST" ]]; then
                log "INFO" "Emergency Recovery LaunchAgent 실행"
                launchctl load -w "$EMERGENCY_PLIST" 2>/dev/null || true
                launchctl start ai.openclaw.emergency-recovery
                RECOVERY_METHOD="LaunchAgent"
                date +%s > "$RECOVERY_START_FILE"

                send_alert "warning" "⚠️ Level 3 Emergency Recovery 시작" \
                    "doctor --fix 실패로 Claude Code 자율 복구 시작\n\nMethod: LaunchAgent\n진행 상황은 Discord로 알림됩니다." \
                    "[{\"name\":\"doctor --fix\",\"value\":\"${attempts}/${DOCTOR_FIX_MAX_ATTEMPTS} 실패\",\"inline\":true}]"
            else
                # Option 2: 직접 실행 (Fallback)
                EMERGENCY_SCRIPT="$HOME/openclaw/scripts/emergency-recovery.sh"
                if [[ -x "$EMERGENCY_SCRIPT" ]]; then
                    log "INFO" "Emergency Recovery 스크립트 직접 실행: $EMERGENCY_SCRIPT"
                    nohup "$EMERGENCY_SCRIPT" > "$LOG_DIR/emergency-recovery-trigger.log" 2>&1 &
                    RECOVERY_PID=$!
                    log "INFO" "Emergency Recovery PID: $RECOVERY_PID"
                    RECOVERY_METHOD="nohup (PID: $RECOVERY_PID)"
                    date +%s > "$RECOVERY_START_FILE"
